...
if(!global.pause && !dead) {
	...
	if(_entity_number > 0) {
		for(var _i = 0; _i < _entity_number; _i++) {
			var _entity_index = ds_list_find_value(_entity, _i);
			if(_entity_index && y_height >= _entity_index.y_height - 30 && y_height <= _entity_index.y_height + 30 && _entity_index.entity_sprite.image_index < _entity_index.entity_sprite.image_number - 2 && _entity_index.attacking && !damaged) {
				if(_entity_index.x < x && _entity_index.last_horizontal_direction == 1 || _entity_index.x > x && _entity_index.last_horizontal_direction == -1) {
					if(!(_entity_index.player_controlled && player_controlled) && !(!_entity_index.player_controlled && !player_controlled)) {
						damaged = true;
						if(attacking) {
							attacking = false;
							attack_id = 0;
